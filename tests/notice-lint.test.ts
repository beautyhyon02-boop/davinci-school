...
describe('lintNotice', () => {
  ...
  it('checks every result of a 단원 평가 차시 (서술형 + 논술형): the second result is linted too and names its kind', () => {
    const two = structuredClone(base)
    two.essay_results.push({ kind: '논술형', confirmed_score: 17, total_points: 16, band: '상', criteria_feedback: [{ criterion_name: '해석', band_score: 3, max: 4, good_point: '', improve_point: '박OO보다 늦음' }], retry: { attempted: true, before_score: 10, after_score: 12, improvement_comment: '실패를 딛고 올림' } })
    const issues = lintNotice(two, ['박OO'])
    expect(issues.some((i) => i.startsWith('논술형: 확정 점수가 만점'))).toBe(true)
    expect(issues.some((i) => i.includes('해석: 잘한 점 없이'))).toBe(true)
    expect(issues.some((i) => i.startsWith('논술형 재도전') && i.includes('실패'))).toBe(true)
    expect(issues.some((i) => i.includes('박OO'))).toBe(true)
  })
  it('flags negative verbs, rankings, other students, improve without good, non-suggestive home study, score over max', () => {
    ...
    bad.essay_results[0].criteria_feedback[0].good_point = ''
    bad.essay_results[0].criteria_feedback[0].improve_point = '박OO보다 등수가 낮음'
    bad.next_lesson.home_study_suggestion = '더 열심히'
    bad.essay_results[0].confirmed_score = 5
  ...
  it('checks the director fields too, flags a lone verdict ending and an empty good point, ignores the student own name', () => {
    ...
    bad.essay_results[0].criteria_feedback[0].good_point = ''; bad.essay_results[0].criteria_feedback[0].improve_point = null
