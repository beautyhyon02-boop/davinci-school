...
import { runStage, runThemeIntro, StageError, STAGE_ERRORS, type Repo, type ThemeRepo, type StageStatus, type ThemeLogRow, type ThemeIntroAction } from '@/lib/studio/stages'
...
// 대주제 소개(0단계)는 검토 관문이 없다(2026-09-24 오너 결정) — 생성(AI 초안)·저장 두 동작뿐이다.
describe('runThemeIntro', () => {
  it('generate makes a draft and does not save theme.intro', async () => {
    ...
    expect(g.status.output).toBeTruthy()
    expect(repo.intro).toBeNull()
  })

  it('generate can be repeated freely — there is no review limit to block it', async () => {
    const repo = fakeThemeRepo()
    for (let i = 0; i < 5; i++) {
      const g = await runThemeIntro({ themeId: 't1', action: 'generate', repo })
      expect(g.status.state).toBe('generated')
  ...
  it('save validates and stores an accepted intro immediately (no review step)', async () => {
    const repo = fakeThemeRepo()
    const intro = '학교 축제에서 나오는 일회용품을 살펴본다. 과목마다 중1 수준에서 할 수 있는 활동을 찾는다. 결론은 학생이 직접 내린다.'
    const s = await runThemeIntro({ themeId: 't1', action: 'save', repo, edit: {
      intro,
      subject_ideas: [{ subject: '수학', idea: '설문 결과를 도수분포표로 정리한다' }, { subject: '과학', idea: '플라스틱의 성질을 관찰한다' }],
    } })
    expect(s.status.state).toBe('accepted')
    expect(s.status.model).toBe('manual')
    expect(s.status.review).toEqual({ pass: true, issues: [] })
    expect(repo.intro).toBe(intro)
  })

  // 화면(ThemeIntroPanel)은 대주제 과목(수학·과학)의 아이디어만 저장 요청에 담는다 — 초안에 다른 과목 아이디어가
  // 섞여 있어도(fixture가 5과목 데모용이라 그렇다) 그 과목들은 화면에 칸이 없어 저장되지 않는다.
  function pickThemeSubjectIdeas(output: { intro: string; subject_ideas: { subject: string; idea: string }[] }) {
    return {
      intro: output.intro,
      subject_ideas: ['수학', '과학'].map((subject) => ({ subject, idea: output.subject_ideas.find((i) => i.subject === subject)?.idea ?? '' })),
    }
  }

  it('save right after a generate keeps the AI model name when the saved text matches the draft exactly', async () => {
    const repo = fakeThemeRepo()
    const g = await runThemeIntro({ themeId: 't1', action: 'generate', repo })
    const draft = g.status.output as { intro: string; subject_ideas: { subject: string; idea: string }[] }
    const s = await runThemeIntro({ themeId: 't1', action: 'save', repo, edit: pickThemeSubjectIdeas(draft) })
    expect(s.status.model).toBe(g.status.model)
    expect(s.status.model).not.toBe('manual')
  })

  it('save with an edited draft is recorded as manual', async () => {
    const repo = fakeThemeRepo()
    const g = await runThemeIntro({ themeId: 't1', action: 'generate', repo })
    const draft = g.status.output as { intro: string; subject_ideas: { subject: string; idea: string }[] }
    const picked = pickThemeSubjectIdeas(draft)
    const s = await runThemeIntro({ themeId: 't1', action: 'save', repo, edit: { ...picked, intro: `${picked.intro} 조금 고쳤다.` } })
    expect(s.status.model).toBe('manual')
  })

  it('save with an empty intro is refused with a clear invalid-edit error and saves nothing', async () => {
    const repo = fakeThemeRepo()
    const p = runThemeIntro({ themeId: 't1', action: 'save', repo, edit: { intro: '   ', subject_ideas: [{ subject: '수학', idea: '설문 결과를 정리한다' }] } })
  ...
  it('save refuses ideas for a subject that is not in the theme, and an empty idea', async () => {
    const repo = fakeThemeRepo()
    const intro = '학교 축제에서 나오는 일회용품을 살펴보는 대주제입니다.'
    await expect(runThemeIntro({ themeId: 't1', action: 'save', repo, edit: { intro, subject_ideas: [{ subject: '영어', idea: '영어 설문을 만든다' }] } }))
      .rejects.toMatchObject({ code: STAGE_ERRORS.INVALID_EDIT })
    await expect(runThemeIntro({ themeId: 't1', action: 'save', repo, edit: { intro, subject_ideas: [{ subject: '수학', idea: '' }] } }))
      .rejects.toMatchObject({ code: STAGE_ERRORS.INVALID_EDIT })
  })

  it('review and accept are no longer supported actions', async () => {
    const repo = fakeThemeRepo()
    await expect(runThemeIntro({ themeId: 't1', action: 'review' as ThemeIntroAction, repo }))
      .rejects.toMatchObject({ code: STAGE_ERRORS.INTRO_ACTION_NOT_SUPPORTED })
    await expect(runThemeIntro({ themeId: 't1', action: 'accept' as ThemeIntroAction, repo }))
      .rejects.toMatchObject({ code: STAGE_ERRORS.INTRO_ACTION_NOT_SUPPORTED })
    expect(repo.intro).toBeNull()
  })
})
...
describe('accepted theme intro reaches stage 2~6 prompts', () => {
  ...
  it('stage 2 prompt carries the accepted (saved) intro text', async () => {
  ...
  it('stage 2 prompt omits an intro that is only a draft (not saved), and one that does not exist', async () => {
    const drafted = await ctxAfterStage2Generate(introStatus('generated'))
    expect(drafted.prior.stage0).toBeUndefined()
    expect(buildPrompt(2, drafted).user).not.toContain(INTRO_TEXT)
