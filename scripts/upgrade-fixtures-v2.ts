  ...
  },
  {
    ...
    patchLessons: (lessons) => {
      ...
      teachFormerEssayLesson(lessons[4], {
        ...
        quiz: [
          // fix wave 2: 예전 Q1 의 정답("미생물이 거의 분해하지 못해 오래 남는다")은 논술형의 과학적 근거 그 자체라 분리배출 표시 읽기로 바꾼다
          { q: '컵 바닥에 "PP"라는 분리배출 표시가 있다. 이 표시가 알려 주는 것은?', type: 'choice', choices: ['컵을 만든 플라스틱의 종류', '컵을 쓸 수 있는 횟수', '컵이 분해되는 데 걸리는 기간'], answer: '컵을 만든 플라스틱의 종류', explanation: '분리배출 표시는 재질(플라스틱의 종류)을 알려 주어 같은 종류끼리 모을 수 있게 한다.' },
