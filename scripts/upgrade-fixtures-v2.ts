...
const SETS: SetDef[] = [
  {
    ...
    patchLessons: (lessons) => {
      ...
      // 5차시: 1은 종이컵 상대도수 계산(C)으로 남긴다. 2(수업 예시 432개를 되풀이)는 자료 A에 상대도수를 쓰는 새 사례(B)로, 3은 전개 ③ 문장을
      // 옮긴 발문이라([TS] 수업 흐름) 다른 말로 묻는다(D~E). 논술형의 품목·목표 수치는 말하지 않는다(C-03).
      setLeveledQuizzes(lessons, 5, ['작년 종이컵의 상대도수를', '올해 종이컵을 작년 상대도수', '근거로 쓸 수치가 어느 자료에서'], [
        'C',
        ['B', '자료 A에서 일회용컵을 40개 이상 쓴 부스의 상대도수는? (부스 20곳 기준)', '0.5 / 0.50', '40개 이상인 부스는 5 + 4 + 1 = 10곳이다. 10 ÷ 20 = 0.5이다.'],
  ...
  },
  {
    ...
    patchLessons: (lessons) => {
      ...
      // 3차시: 물에 뜬 조각의 재질(관찰 회상) · 새 사례를 재활용이 어려운 까닭에 맞추기(적용) · 공장이 조각을 잘게 부수는 까닭(관찰 원리를 새 상황에).
      ...
      setLeveledQuizzes(lessons, 3, ['컵 안에 음료가 남아 있거나', '물에 넣었더니 PET 조각은', '종이컵은 안쪽에 얇은 막이'], [
        ...
        ['B', '재활용 공장이 물에 뜨고 가라앉는 차이로 플라스틱을 나눌 때, 컵을 통째로 넣지 않고 먼저 어떻게 하는가?', '잘게 부순다 / 부순다 / 잘게 자른다 / 조각낸다', '통째로 넣으면 컵 안의 공기 때문에 무엇이든 뜬다. 잘게 부순 조각이어야 재질마다 뜨고 가라앉는 차이가 드러난다.'],
      ])
