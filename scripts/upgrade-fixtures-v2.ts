...
/** 수준을 붙인 단답형 퀴즈: [수준(L-10), 발문, 정답, 해설 1~2문장]. */
type LeveledQuizDef = [level: QuizLevel, q: string, answer: string, explanation: string]
/**
 * 대표 2026-09-26(L-10, "차시 설계나 퀴즈가 너무 쉬운 수준이 아닌지"): 교수 차시 퀴즈 3문항을 성취수준 틀에 묶는다 — D~E 회상(용어·사실),
 * C 이해·적용(도달점 수준의 적용·계산), B 관계·추론(두 개념의 관계, 까닭, 새 사례) 하나씩(level_ref). v1 에서 올린 1~4차시 퀴즈는
 * compat 이 같은 문항으로 발문 대본·활동지 기본·표준 과제를 만들어(upgradeLessonV1) 수업에서 이미 물은 문항을 되풀이했고, 몇몇은 자료 D
 * 문장을 옮겨 적으면 답이 됐다([TS] "정답이 본문에 그대로 있음"). 발문·활동지는 수업용으로 그대로 두고 퀴즈만 새 사례·수치로 다시 쓴다.
 * defs[i] 가 수준 하나면 그 퀴즈를 남기고 수준만 붙인다. olds[i] 는 지금 퀴즈 발문의 앞부분 — 다르면(입력이 바뀌었으면) 멈춘다.
 * 서·논술형의 답(수학 0.24·0.30과 상대도수로 비교하는 이유, 과학 PS 예측·일회용컵 제안)은 말하지 않는다(C-03).
 */
function setLeveledQuizzes(lessons: LessonT[], no: number, olds: [string, string, string], defs: [QuizLevel | LeveledQuizDef, QuizLevel | LeveledQuizDef, QuizLevel | LeveledQuizDef]) {
  const l = lessons.find((x) => x.no === no)
  const quiz = l?.formative_check.quiz ?? []
  if (!l || quiz.length !== 3) throw new Error(`PATCHES: ${no}차시 퀴즈가 3문항이 아님`)
  l.formative_check.quiz = quiz.map((old, i) => {
    if (!old.q.startsWith(olds[i])) throw new Error(`PATCHES: ${no}차시 퀴즈 ${i + 1}이 "${olds[i]}…"가 아님`)
    const d = defs[i]
    if (typeof d === 'string') return { ...old, level_ref: d }
    const [level, q, answer, explanation] = d
    return { ...shortQuiz([q, answer, explanation]), level_ref: level }
  })
}
...
const SETS: SetDef[] = [
  {
    ...
    patchLessons: (lessons) => {
      ...
      // 대표 2026-09-26(L-10): 퀴즈 수준 D~E·C·B 하나씩(setLeveledQuizzes 주석). 1~4차시는 발문·활동지와 같은 문항이던 퀴즈를 새 사례로 다시 쓴다.
      // 1차시: 탐구 문제(다른 상황으로 회상) · 조사 대상(새 질문에 적용) · 집단의 크기가 다르면 개수로 비교할 수 없다(새 사례의 관계 — 상대도수로 비교하는 이유 문장은 쓰지 않는다)
      setLeveledQuizzes(lessons, 1, ['"쓰레기가 너무 많다"', '자료 A에서 "조사 대상"', '자료 B처럼 작년과 올해'], [
        ['D~E', '"우리 반 학생들은 하루에 물을 몇 컵씩 마시는가?"처럼 자료를 모아 정리하고 해석해서 답을 찾는 질문을 무엇이라 하는가?', '통계적 탐구 문제 / 탐구 문제', '마신 컵 수라는 자료를 모아 답하므로 통계적 탐구 문제이다. 느낌을 말한 질문과 다르다.'],
        ['C', '"우리 학교 1학년 학생들은 하루에 일회용컵을 몇 개씩 쓰는가?"를 조사할 때 조사 대상은 누구인가?', '우리 학교 1학년 학생 / 1학년 학생 / 1학년 학생들', '자료를 얻을 사람이나 사물이 조사 대상이다. 이 질문은 우리 학교 1학년 학생들을 조사한다.'],
        ['B', 'A반은 30명이 우유 90개를, B반은 20명이 우유 70개를 마셨다. 개수는 A반이 많지만 한 명당 마신 개수는 B반이 A반보다 몇 개 더 많은가?', '0.5 / 0.5개', '한 명당 A반은 3개, B반은 3.5개이다. 집단의 크기가 다르면 개수만으로 비교할 수 없다.'],
      ])
      // 2차시: 계급(용어 회상) · 자료 A의 다른 계급 도수(적용) · 새 자료를 계급의 크기 5로 나누기(경계값 5·15가 어느 계급인지 따지는 새 사례)
      setLeveledQuizzes(lessons, 2, ['자료 A를 줄기와 잎 그림으로', '계급 30개 이상 40개 미만의 도수는', '2차시 도수분포표는'], [
        ['D~E', '도수분포표에서 "20개 이상 30개 미만"처럼 변량을 일정한 간격으로 나눈 구간을 무엇이라 하는가?', '계급', '변량을 나눈 구간이 계급이고, 구간의 너비가 계급의 크기이다.'],
        ['C', '자료 A를 계급의 크기 10으로 나눈 도수분포표에서 계급 50개 이상 60개 미만의 도수는?', '4 / 4곳', '51, 52, 55, 58로 모두 4곳이다. 60개인 부스가 있다면 다음 계급에 넣는다.'],
        ['B', '어느 반 10명의 턱걸이 기록 3, 5, 7, 8, 12, 13, 15, 18, 21, 24(회)를 0회부터 계급의 크기 5로 나누면, 도수가 가장 큰 계급은?', '5회 이상 10회 미만 / 5 이상 10 미만', '5·7·8회 세 명이 들어간다. 5회는 "이상"이라 이 계급에, 15회는 "미만"이라 다음 계급에 넣는다.'],
      ])
      // 3차시: 도수분포다각형 점의 높이(회상) · 자료 A 히스토그램 읽기(적용) · 히스토그램과 도수분포다각형의 넓이 관계(두 개념의 관계)
      setLeveledQuizzes(lessons, 3, ['히스토그램에서 직사각형의 세로 길이는', '자료 A의 히스토그램에서 가장 높은', '히스토그램에서 각 직사각형 윗변의'], [
        ['D~E', '도수분포다각형에서 각 점의 높이는 무엇을 나타내는가?', '도수', '점의 높이는 그 계급의 도수이다. 점은 히스토그램 직사각형 윗변의 가운데에 찍는다.'],
        ['C', '자료 A의 히스토그램(계급의 크기 10)에서 두 번째로 높은 직사각형의 계급은?', '40개 이상 50개 미만 / 40 이상 50 미만', '도수는 30개 이상 40개 미만이 6으로 가장 크고, 40개 이상 50개 미만이 5로 그다음이다.'],
        ['B', '자료 A의 히스토그램에서 직사각형 넓이의 합은 200이다. 같은 자료의 도수분포다각형과 가로축으로 둘러싸인 부분의 넓이는?', '200', '두 넓이는 같다. 다각형이 잘라 낸 삼각형과 새로 덮은 삼각형의 넓이가 서로 같기 때문이다.'],
      ])
      // 4차시: 상대도수(용어 회상) · 새 자료의 상대도수 계산(적용) · 상대도수와 도수·총합의 관계를 거꾸로 쓰기(관계). 플라스틱컵 값(0.24·0.30)은 쓰지 않는다
      setLeveledQuizzes(lessons, 4, ['올해 일회용 접시의 상대도수는', '작년과 올해처럼 도수의 총합이', '상대도수의 총합은'], [
        ['D~E', '어떤 계급의 도수가 전체 도수에서 차지하는 비율을 무엇이라 하는가?', '상대도수', '상대도수는 그 계급의 도수를 도수의 총합으로 나눈 값이다.'],
        ['C', '어느 반 학생 25명 가운데 걸어서 등교하는 학생이 9명이다. 걸어서 등교하는 학생의 상대도수는?', '0.36', '9 ÷ 25 = 0.36이다. 상대도수는 도수를 도수의 총합으로 나눈다.'],
        ['B', '도수의 총합이 50인 자료에서 어떤 계급의 상대도수가 0.2이다. 이 계급의 도수는?', '10 / 10개', '도수 = 상대도수 × 도수의 총합이므로 0.2 × 50 = 10이다.'],
      ])
      // 5차시: 1은 종이컵 상대도수 계산(C)으로 남긴다. 2(수업 예시 432개를 되풀이)는 개수와 상대도수가 반대로 말하는 두 집단의 새 사례(B)로 —
      // 리뷰 round 2: "자료 A의 40개 이상 상대도수"는 1(C)과 같은 기능(개수 ÷ 전체)을 되풀이해 바꿨다. 3은 전개 ③ 문장을 옮긴 발문이라([TS] 수업 흐름)
      // 다른 말로 묻는다(D~E). 논술형의 품목·목표 수치와 플라스틱컵 값은 말하지 않는다(C-03).
      setLeveledQuizzes(lessons, 5, ['작년 종이컵의 상대도수를', '올해 종이컵을 작년 상대도수', '근거로 쓸 수치가 어느 자료에서'], [
        'C',
  ...
  },
  {
    ...
    patchLessons: (lessons) => {
      ...
      // 대표 2026-09-26(L-10): 퀴즈 수준 D~E·C·B 하나씩(setLeveledQuizzes 주석). 1~4차시는 발문·활동지와 같은 문항이던 퀴즈(자료 D 문장을 옮기면
      // 답이 되던 2차시 포함)를 새 사례로 다시 쓴다.
      // 1차시: 탐구 단계 이름(회상) · 새 활동이 어느 단계인지(적용) · 가설을 확인하려면 무엇을 세어야 하는지(가설과 자료의 관계)
      setLeveledQuizzes(lessons, 1, ['과학적 탐구 방법은', '과학적 탐구 문제는 느낌이', '"다회용컵을 빌려주면'], [
        ['D~E', '실험·관찰·조사로 얻은 결과를 보고 가설이 맞는지 판단하는, 과학적 탐구 방법의 마지막 단계를 무엇이라 하는가?', '결론 도출 / 결론 내리기 / 결론', '모은 자료로 가설이 맞는지 판단하는 단계가 결론 도출이다.'],
        ['C', '한 모둠이 가설을 확인하려고 두 부스의 일회용컵 개수를 하루 동안 세었다. 개수를 센 활동은 탐구 과정의 어느 단계인가?', '자료 수집 / 자료 수집 단계 / 자료를 수집한다', '가설을 확인할 결과를 세거나 재서 모으는 활동이므로 자료 수집이다.'],
        ['B', '가설 "부스에 다회용컵을 빌려주면 일회용컵 사용이 줄어들 것이다"를 확인하려면, 빌려준 부스와 빌려주지 않은 부스에서 무엇을 세어 견주어야 하는가?', '일회용컵 사용 개수 / 일회용컵 개수 / 일회용컵 수', '가설이 예상한 결과가 일회용컵 사용이 줄어드는 것이므로, 두 경우의 일회용컵 개수를 세어 견준다.'],
      ])
      // 2차시: 분해(용어 회상) · 장점이 버려진 뒤 문제가 되는 성질(설명 핵심어) · 나무와 플라스틱이 썩는 데 걸리는 시간이 다른 까닭(새 사례의 까닭)
      setLeveledQuizzes(lessons, 2, ['자료 D에 따르면 플라스틱은 무엇에서', '컵 바닥에 찍힌 PET, PP, PS', '자료 D에 따르면 땅에 묻힌'], [
        ['D~E', '생물이 물질을 작게 쪼개어 자연으로 돌려보내는 일을 무엇이라 하는가?', '분해', '분해는 생물이 물질을 작게 쪼개 자연으로 돌려보내는 일이다.'],
        ['C', '쓸 때는 플라스틱의 장점이지만, 버려진 뒤에는 오래 남는 문제의 원인이 되는 성질은? ("잘 ~ 않는다" 꼴)', '잘 변하지 않는다 / 변하지 않는다', '잘 변하지 않아서 버려진 뒤에도 오래 남는다. 쓸 때는 오래 쓸 수 있는 장점이다.'],
        ['B', '땅에 묻힌 나무젓가락은 비교적 빨리 썩지만 플라스틱컵은 오래 남는다. 나무는 분해하지만 플라스틱은 거의 분해하지 않는 생물은?', '미생물', '나무는 미생물이 분해해 흙으로 돌아간다. 플라스틱은 미생물이 거의 분해하지 않아 오래 남는다.'],
      ])
      // 3차시: 물에 뜬 조각의 재질(관찰 회상) · 새 사례를 재활용이 어려운 까닭에 맞추기(적용) · 두 재질이 붙은 컵은 뜨고 가라앉는 차이로 나뉘지 않아 먼저
      // 떼어 내야 한다(관찰 원리 + 재질 섞임의 관계). 리뷰 round 2: 이전 B("공장이 컵을 잘게 부순다")는 3차시에서 가르치지 않은 전제에 기대 바꿨다.
      // 서술형 새 사례(PS 컵의 뜨고 가라앉음)는 쓰지 않는다
      setLeveledQuizzes(lessons, 3, ['컵 안에 음료가 남아 있거나', '물에 넣었더니 PET 조각은', '종이컵은 안쪽에 얇은 막이'], [
        ['D~E', '컵 조각을 물에 넣는 관찰에서 PET 조각은 가라앉았다. 물 위로 뜬 조각의 재질 표시는?', 'PP', 'PP는 같은 부피일 때 물보다 가벼워 물에 뜬다. PET는 물보다 무거워 가라앉는다.'],
        ['C', '종이 상자에 비닐 테이프가 많이 붙어 있으면 재활용이 어려워진다. 재활용을 어렵게 하는 세 까닭(오염·재질 섞임·색소와 첨가제) 가운데 무엇에 해당하는가?', '재질 섞임 / 서로 다른 재질 / 다른 재질이 섞임', '종이와 비닐처럼 서로 다른 재질이 붙어 있으면 종류별로 모을 수 없다.'],
        ['B', '종이컵은 안쪽에 얇은 플라스틱(PE) 막이 붙어 있어, 조각을 물에 넣어 뜨고 가라앉는 차이로 두 재질을 가를 수 없다. 이런 컵을 재활용하려면 먼저 두 재질을 어떻게 해야 하는가?', '떼어 낸다 / 분리한다 / 재질을 분리한다 / 따로 떼어 낸다', '두 재질이 한 조각에 붙어 있으면 물에서 함께 움직여 나뉘지 않는다. 먼저 떼어 내야 재질별로 모을 수 있다.'],
      ])
      // 4차시: 지속가능한 삶(다른 말로 회상) · 새 방안의 차원 구분(적용) · 자료 E의 무게로 두 컵의 재료량 견주기(관계 — 언제 나은지 결론은 말하지 않는다)
      setLeveledQuizzes(lessons, 4, ['지금 세대의 필요를', '학생회가 다회용컵 대여와', '섞여 들어온 플라스틱을'], [
        ['D~E', '자원을 아껴 쓰고 환경을 지켜 미래 세대도 지금처럼 살아갈 수 있게 하는 삶의 방식을 무엇이라 하는가?', '지속가능한 삶 / 지속가능성', '지금의 필요와 미래 세대의 삶을 함께 생각하는 삶이 지속가능한 삶이다.'],
        ['C', '마트에 갈 때 장바구니를 챙겨 비닐봉지를 덜 쓰는 것은 개인 차원과 사회 차원 가운데 어느 차원의 방안인가?', '개인 / 개인 차원', '나 혼자 정해서 할 수 있는 일이므로 개인 차원의 방안이다.'],
        ['B', '자료 E에서 스테인리스 다회용컵(약 150 g)을 만드는 재료는 일회용 PET컵(약 8 g) 몇 개를 만드는 양과 비슷한가? (반올림해 자연수로)', '19 / 약 19개 / 19개', '150 ÷ 8 = 18.75이므로 약 19개이다. 재사용 횟수와 함께 견주어 볼 수치이다.'],
      ])
      // 5차시: 1(자료 D, D~E)·2(비닐봉지 개수 차이, C)는 남긴다. 3(단위 용어)은 비닐봉지도 플라스틱이라 자료 D의 과학적 이유를 쓸 수 있다는
      // 새 사례의 관계(B)로 바꾼다. 일회용컵 제안 내용은 말하지 않는다(C-03).
      setLeveledQuizzes(lessons, 5, ['제안서 뼈대의 "과학적 이유" 칸', '자료 B에서 비닐봉지는', '자료 B의 수치를 근거로 쓸 때'], [
        'D~E',
        'C',
