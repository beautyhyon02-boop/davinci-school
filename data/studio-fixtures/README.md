...
**퀴즈 수준(대표님 2026-09-26, 규칙 L-10)**: 교수 차시 퀴즈 30문항(두 세트 × 교수 차시 5 × 3)에 `level_ref`(D~E 회상 · C 이해·적용 · B 관계·추론)가 차시마다 하나씩 붙어 있다. v1에서 올린 퀴즈는 발문 대본·활동지 기본·표준 과제와 같은 문항이어서(compat가 v1 퀴즈로 발문·활동지를 만든다) 27문항을 새 사례·수치로 다시 썼고(발문·활동지는 수업용으로 그대로), 남긴 3문항(수학 5차시 1, 과학 5차시 1·2)에는 수준만 붙였다 — `scripts/upgrade-fixtures-v2.ts` `setLeveledQuizzes`(차시마다 바꾼 까닭 주석). `tests/mock-fixtures.test.ts`가 차시마다 D~E·C·B 하나씩, 정답이 그 차시 본문(수업 흐름·발문 대본·활동지·사용 자료)에 그대로 있지 않음([TS] `quizCopySource`), 발문·활동지 문항을 되풀이하지 않음을 검사한다.
