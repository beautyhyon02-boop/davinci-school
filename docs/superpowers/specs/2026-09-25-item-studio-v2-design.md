...
export const Condition = z.object({
  ...
  text: z.string().min(5),                       // "…를 각각 서술할 것. (1점)" 문장 그대로(학생이 봄)
  verb: z.string().min(1),                       // 행동 동사 원형: 구하다·판단하다·설명하다·제시하다·비교하다 …
...
export const Criterion = z.object({
  name: z.string().min(2),
  axis: z.enum(['지식·이해', '과정·기능', '가치·태도']),
  condition_nos: z.array(z.number().int()).min(1),   // 이 요소가 채점하는 조건 번호(조건↔채점표 대응)
...
3. 조건의 `points` 합 ≤ 문항 배점; 모든 조건 번호가 어느 요소의 `condition_nos`에 한 번 이상 등장(조건은 반드시 채점표에 반영). 조건이 4개 이상이면 `category`가 두 종류 이상.
...
[AI] 검토 초점: 채점표로 예시답안을 실제로 채점해 적힌 점수가 나오는지(v1 유지, 이제 문항마다), 조건이 학생 혼자 답안을 쓸 수 있을 만큼 구체적인지(v1), 자료가 답을 대신하지 않는지, 발문 사고 순서 = 조건 순서, 성취기준 이탈, 학년 수준, 척도 descriptor가 관찰 가능한 표현인지, 논술형 4요소 중 최소 1개가 가치·태도 축이면 "정당화 가능성" 기준으로 쓰였는지, 예시 은행 문장을 그대로 베끼지 않았는지.
...
| ID | 규칙 | 성격 | 출처 |
|---|---|---|---|
| C-01 | 문항은 발문·자료·조건 세 요소로 구성하고, 조건에 담은 제약은 반드시 채점표 요소에 대응시킨다(`condition_nos`) | P+S | [WP4-1][WP13-5][WP5-국-2][WP5-사-2] |
...
| C-11 | 조건은 행동 동사 단위로 쪼개고 필요한 조건 끝에 부분배점을 소괄호로 병기한다; 문항 배점은 문두 끝 대괄호 | P+S | [서울-3][WP5-수-2][WP5-사-2] |
...
| S-수-04 | 서술형 "이유 서술"은 핵심 채점 포인트를 조건에 명시한다(예: 총합이 다르다는 점을 언급) | [WP11-5] |
...
| S-사-02 | 조건 3~5개가 채점 요소와 1:1, 글자 수 조건은 ±50자 | [WP5-사-2] |
...
| S-사-06 | "자료 내용을 그대로 옮기지 말 것"을 조건에 넣고 재진술을 채점 | [2025-2] |
| S-역-01 | 사료는 상반된 행위자 관점의 1차 사료 2개 이상, "(가)에서 인용할 것" 조건과 인용 정확성 요소 | [WP4-11][WP4-12][2025-2] |
...
| S-과-03 | 필수 용어(예: 항체·전기음성도)를 조건에 명시하고 충족 여부로 배점 | [WP5-과-3] |
