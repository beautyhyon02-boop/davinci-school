...
- 검토 규칙: [TS] 표 합계·비율 일관성(3B 그대로). [AI] 학교급(학년군) 어휘, 자료가 결론을 담지 않는지.
...
- 검토 규칙: [AI] 학교급 적합성 — 성취기준 코드가 학교급 접두(중 `[9…]`, 초 `[4…]`·`[6…]`)인지와 대주제 적합성만 본다. 학년군 안에서 몇 학년 내용인지는 따지지 않는다(대표 2026-09-26, 대주제 학년은 선택). [TS] `getLevels`가 `null`이면 경고(도덕 등 DB 밖 과목, `coverage.md`).
...
검토 규칙:
- [TS] 모든 성취기준이 어느 차시엔가 배정(v1 coverage). `summative_placement`와 `lessons[].assessment`가 일치. 단원 평가 차시(`kind: 'assessment'`)가 정확히 1개이고 마지막 번호이며 `['서술형', '논술형']`을 담고, 교수 차시에는 서·논술형이 없고, 교수 차시는 3~5개(`sessionPlacementIssues`). 평가 차시 전개에 서술형 작성 소단계와 논술형 작성(35분 이상) 소단계. `mergeable_with`는 인접 번호이고 단원 평가 차시는 병합하지 않음. 퀴즈는 모두 단답형(`type: 'short'`, `choices: null`) — 선택지가 있으면 "퀴즈는 단답형만(선택지 금지)"(대표 2026-09-26: 객관식 폐지; 새 세트는 zod `LessonDesign`도 막고, 게시 판 읽기 `PublishedLessonDesign`만 옛 선택형을 받는다). 활동지 `tier` 세 종류가 모두 있음. `materials_used`가 4단계 자료 ID 범위(A~F) 안.
...
- [AI] 자료가 문항이 요구할 결과(정리된 도수분포표·계산된 상대도수·결론 문장)를 미리 담지 않았는지(v1 `other`), 찬반·비교 자료의 균형, 학교급(학년군) 어휘, 자료 간 관계(보완·대비)가 문항 사고 유형과 맞는지.
...
[AI] 검토 초점: 채점표로 예시답안을 실제로 채점해 적힌 점수가 나오는지(v1 유지, 이제 문항마다), 서술형에 조건이 없고 논술형 조건이 지침만 담는지(풀이 과정·공식·수치·순서가 있으면 other, C-32 — v1의 "학생 혼자 답안을 쓸 만큼 구체적인지"를 대체), 자료가 답을 대신하지 않는지, 발문 사고 순서 = 조건 순서, 성취기준 이탈, 학교급(학년군) 수준, 척도 descriptor가 관찰 가능한 표현인지, 논술형 4요소 중 최소 1개가 가치·태도 축이면 "정당화 가능성" 기준으로 쓰였는지, 예시 은행 문장을 그대로 베끼지 않았는지.
...
태그: `[WP4-n]` wp4 §10 프롬프트 규칙 n, `[WP4-운n]` 설계·운영 지침, `[WP13-n]` wp13 §5, `[WP1-n]` wp1 §7, `[WP11-n]` wp11 §6, `[서울-n]` 서울 §6, `[WP8-n]` wp8 §6, `[WP7]` wp7 §8·§9, `[WP5-과목-n]` README 관찰, `[2025-n]` README-사과 관찰, `[대표]` 대표님 확정, `[대표-연수2기]` 대표 교사 연수 2기 실습-2(2026-09-02 — 핵심질문 6단계·자료 설계, docs/research/2026-09-26-owner-training-deck-2.md), `[경기2025]` 경기도교육청 논술형 평가 장학자료(2025.7, docs/research/corpus/wp9-gyeonggi-essay.md §7 — 대표님 결정 불필요, 2026-09-25 채택), `[v1]` 기존 rules.ts. 성격: **P** 프롬프트 문장, **S** 스키마·검토 코드, **O** 운영 절차.
...
| C-14 | 예시답안은 서술형은 0점을 뺀 총점 단계마다 1개(6점이면 6·5·4·3·2·1), 논술형은 상/중/하 각 1개를 요소별 점수·채점자 의견(rationale)과 함께 만들고(학생이 입장을 선택하는 논술형은 입장별로 예시답안을 각 1편씩 만든다), "유일한 정답이 아님"을 유의점에 적는다. | P+S | [서울-5][WP4-17][WP13-7][WP5-영-5][경기2025] |
...
| C-16 | 채점 시 유의점(notes) 1~4줄: 핵심 내용이 있으면 표현·단어 차이로 감점하지 않음, 맞춤법은 의미 전달되면 관용, 반올림 허용 범위, 그림 대신 말로 설명한 경우 인정 여부. 전용 칸이 없으므로 피드백 시 유의점(성취수준 부합 정도·오개념 대응 방향)도 이 유의점 줄에 함께 적는다. | P+S | [WP13-6][서울-8][WP5-국-5][WP5-사-4][경기2025] |
...
| C-24 | 성취기준 원문은 절대 변형하지 않고, 어휘·자료·문항은 교육과정 학교급(학년군) 수준으로 쓴다(학년을 정한 대주제는 그 학년 수준). 학생 제시문은 "~다" 평서문, 지침·안내는 "~합니다/~하세요". | P+S | [대표][v1][v2-0926] |
...
| C-34 | 논술형 소문항은 사다리로 설계한다 — 앞부분(개념·자료 확인, C~B 목표)의 결과를 뒷부분(판단·제안, A 목표)이 근거로 쓰게 하고, 채점표 요소도 이 순서로 갈라 앞 요소는 확인·이해를, 뒤 요소는 판단·제안을 본다. | P+S | [경기2025] |
| C-36 | 두 하위 요소(X·Y)를 묶은 채점 요소는 중간 점수 하나의 수행 특성(descriptor) 안에 "X만 충족"과 "Y만 충족"을 같은 점수의 두 줄로 나란히 적어(예: "① … / ② …") 채점자가 순서를 임의로 매기지 않게 한다. | P+S | [경기2025] |
| C-37 | 결과물 장르가 있는 논술형(연설문·기사·제안서)의 문항(stem)에는 답안 틀(도입 문장 + 빈 칸 [A][B] 또는 항목 제목)을 줄 수 있다. 줄 때는 틀의 칸마다 채점표 요소 1개를 대응시킨다. | P | [경기2025] |
...
| L-04 | 세트 핵심질문 1개 + 차시 핵심질문(사실 확인형 금지)을 핵심질문 6단계로 뽑는다: 재구성 문장에서 기능어 찾기 → 재구성 문장(무엇을 읽고 무엇을 근거로 무엇을 하는지) → 학생에게 남아야 할 핵심 이해 한 문장 → 질문형 → 수업 전체를 끌고 가는지 점검 → 문항과 구분. 질문형은 두 축 "무엇인가?" + "어떻게 알 수 있는가?"를 한 질문에 묶는다(설명한다→"이 현상은 무엇이며 어떤 원리로 설명할 수 있는가", 비교한다→"두 대상은 무엇이 같고 다르며 어떤 기준으로 비교할 수 있는가", 추론한다→"화자나 필자의 의도는 무엇이며 그렇게 판단할 수 있는 단서는 무엇인가", 파악한다→"이 자료의 핵심 내용은 무엇이며 어떤 정보가 이를 가장 잘 보여 주는가"); 주장한다는 입장 + 근거("나는 어떤 입장을 취할 것이며 그 입장을 뒷받침하는 근거는 무엇인가"), 감상한다는 인상 + 근거("어떤 점이 인상적이며 어떤 표현이 그런 느낌을 만들었는가"). 좋은 핵심질문은 자료를 바꿔도 반복해 쓸 수 있고 차시 활동과 단원 평가 문항을 잇는다; 문항 문장이 아니므로 자료 지시·응답 방식(쓰시오·분량·조건)을 붙이지 않는다. 차시 핵심질문은 도입 제시·전개 상기·정리 재확인. | P | [v1][WP7 §5][대표-연수2기] |
...
| L-08 | 활동지(worksheet): 과제 2~5개를 기본(D~E)·표준(C)·도전(A~B) 층으로(각 1개 이상, 한 장에 함께), 자기평가 1~3문장. 전개 활동은 C(도달점)를 목표로 하고 A~B 학생용 확장 활동(발문 또는 활동지 도전 과제) 1개를 포함한다. | P+S | [WP7 §3(e)][WP1 §5][대표] |
| L-09 | 형성평가가 항상 선행한다: 교수 차시는 마지막 교수 차시까지 모두 마무리 퀴즈 정확히 3문항, 모두 단답형 — 낱말·수치·짧은 구를 직접 쓰는 문항, 선택지 없음(정답·해설; 객관식 폐지). 서술형 1 + 논술형 1은 마지막 교수 차시 뒤 별도 단원 평가 차시(kind "assessment", 60분 = 평가 안내 5 · 서술형 작성 15 · 논술형 작성 35 · 정리 5, 퀴즈 0)에서 함께 본다(대표 2026-09-26). | P+S | [WP7 §6][대표] |
| L-10 | 퀴즈 3문항은 모두 단답형이되 수준을 나눈다: 1문항 회상(D~E: 용어·사실), 1문항 이해·적용(C: 도달점 수준의 적용·계산·설명 핵심어), 1문항 관계·추론(B: 두 개념의 관계, 이유, 새 사례 적용)에 각각 level_ref를 붙인다; 정답이 자료·대본에 그대로 적힌 문장을 옮겨 적는 문항은 금지(회상 문항도 낱말을 묻되 문장 베끼기는 안 됨); 답은 낱말·수치·짧은 구; 정답과 한두 줄 해설을 단다. | P+S | [WP11-3][WP11-9][대표] |
...
| ID | 규칙 | 출처 |
|---|---|---|
| S-국-01 | 자료는 (가)(나) 복수 통합형을 기본으로 하되 중학교는 각 200~400자, 총 2편 이내. | [WP5-국-1][C-19] |
...
| S-수-05 | 오류 찾기형(가상 학생의 틀린 풀이)은 고등 위주 템플릿, 중학교는 선택. | [WP5-수-5][C-09] |
| S-수-06 | 학년을 중1로 정한 대주제는 문장·채점표에 '정당화'를 쓰지 않는다(학년을 정하지 않았으면 성취기준 원문에 '정당화'가 있을 때만 쓴다); 안내된 절차·구체적 조작은 D/E 척도에서만. | [WP1 §4] |
...
| S-과-08 | 과학 '타당성 평가' 소문항(광고 문구·주장 검증)은 과학적 근거 없이 가치 판단만 한 의견을 정답으로 인정하지 않는다는 문구를 채점 시 유의점(rubric.notes)에 넣는다(C-27의 정의적 요소 채점과는 다르다 — 여기는 사실 판단 과제). | [경기2025] |
...
| G-10 | 앞 소문항에서 학생이 잘못 구한 값을 뒤 소문항에서 그대로 이어 써서 옳게 전개했으면, 그 오류는 처음 발생한 요소에서만 감점하고 뒤 요소는 이어 쓴 값을 기준으로 정답 여부를 판단한다(오류 이월 인정). | P | [경기2025] |
