...
- 2026-09-26 대표님: **퀴즈·차시 난이도를 성취수준 틀에 묶는다**("차시 설계나 퀴즈가 너무 쉬운 수준이 아닌지") — 교수 차시 퀴즈 3문항은 모두 단답형이되 D~E 회상(용어·사실) · C 이해·적용(도달점 수준의 적용·계산·설명 핵심어) · B 관계·추론(두 개념의 관계·까닭·새 사례) 하나씩(`QuizItem.level_ref`, 규칙 L-10 개정 — "워밍업 어투" 삭제), 자료·대본 문장을 옮겨 적으면 답이 되는 퀴즈 금지; 전개는 C(도달점) 목표 + A~B 학생용 확장 활동 1개(발문 또는 활동지 도전 과제, L-08). 3단계 프롬프트는 수준을 요구하지만, **아무것도 막지 않는다**(대표 결정: 마법사는 멈추지 않는다) — zod는 어느 경로에서도 `level_ref`를 요구하지 않고(선택 필드), 수준이 없거나 고르지 않으면 [TS] 참고 메모("N차시 퀴즈 수준 표시(level_ref)가 없거나 D~E/C/B가 고르지 않음 — 3단계를 다시 생성하면 채워집니다")만 남는다. 베끼기도 참고 메모("N차시 퀴즈 k: 정답이 본문에 그대로 있음(발문 대본·수업 흐름·활동지·자료 X)"). 수준은 교사용 — 원장·관리자 차시 카드의 교사용 지침 칸 "퀴즈 수준" 줄에만 보이고 학생 화면에는 없다. 시연 fixture: v1에서 올린 1~4차시 퀴즈가 발문 대본·활동지와 같은 문항(compat가 v1 퀴즈로 발문·활동지를 만든다)이라 퀴즈를 새 사례·수치로 다시 써(27문항, 발문·활동지는 수업용으로 그대로) 30문항 모두 수준을 붙였다(`scripts/upgrade-fixtures-v2.ts` `setLeveledQuizzes`). 이 변경 전에 3단계를 생성해 저장한 초안(`level_ref` 없음)도 차시 이미지 첨부·3단계 문장 고치기·확인·게시가 그대로 되고, 위 참고 메모만 보인다 — 수준을 채우려면 3단계를 다시 생성한다. 게시·배정된 판은 영향 없다.
...
- 2026-09-26 보완(같은 관찰 후속): 3단계 차시 `materials_used`도 이 과목에 꼭 필요한 것만(3단계 과제·검토, [TS] 자문 "N차시가 문항·활동지가 쓰지 않는 공유 자료 X를 가리킴" — `sharedMaterialCitationIssues`), 단원 평가 차시 `materials_used`는 5단계 문항과 항상 같게 자동 동기화(`syncAssessmentSessionMaterials`, `lib/studio/assessment-structure.ts` — `buildSnapshot`/`upgradeDraftColumns`/5단계 저장(`repo.ts`·`saveStageEdit`)에서 적용, 옛 초안도 재생성 없이 다음 미리보기·게시 때 바로잡힌다). 런북 C절 7-1 보완.
...
- 2026-09-25 (WP9 경기 논술형 장학자료, 대표님 결정 불필요): 경기도교육청 논술형 평가 장학자료(2025.7) 분석에서 나온 규칙안 9건 중 7건을 반영했다(태그 `[경기2025]`) — 새 규칙 **C-34**(논술형 소문항은 사다리로: 앞부분 확인·이해 → 뒷부분 판단·제안), **C-36**(두 하위 요소를 묶은 채점 요소는 중간 점수 하나의 서술 안에 "X만 충족"/"Y만 충족"을 두 줄로), **C-37**(결과물 장르가 있는 논술형은 답안 틀[A][B]을 줄 수 있다, 선택), **G-10**(오류 이월 인정 — 앞 소문항 오류를 그대로 이어 옳게 전개했으면 뒤 요소는 이어 쓴 값 기준으로 채점), **S-과-08**(과학 타당성 평가 소문항은 과학적 근거 없는 가치 판단만으로는 정답 인정하지 않음); 기존 규칙 보강 **C-14**(입장을 선택하는 논술형은 예시답안을 입장별로 각 1편). **C-38**("피드백 시 유의점" 전용 필드안)은 `AssessmentItem` 스키마에 그런 칸이 없어(2026-09-25 확인) 새 필드를 만들지 않고 C-16(`rubric.notes`) 문장에 접어 넣었다 — 전용 칸 분리는 다음 범위. 5단계 검토 초점(`REVIEW_FOCUS[5]`)에 경기 자료의 반면 예시(풀이 힌트가 된 조건 문구 — "미지수를 정하고 일차방정식을 세울 것", "π를 3으로 계산할 것", 값 쌍을 미리 나열한 표)를 반려 기준으로 추가했다. **대표님 결정 대기(반영하지 않음)**: C-35(분량은 "권장"으로만 적고 글자 수로 감점·가점하지 않음 — S-국-03의 ±50자·S-영-02의 단어 구간별 점수와 충돌), C-32 개정안(논술형 조건을 0개까지 허용하는 "확장형" — 현재 C-32는 논술형 2~4개). 분석 전문은 `docs/research/corpus/wp9-gyeonggi-essay.md` §7.
- 2026-09-26 대표님: **5단계 문항은 공개 예시 문항 수준으로** — "예시를 학습했다는데 못 미친다"(카드 900자로 잘려 채점 기준표·예시답안이 모델에 닿지 않았음) → 5단계 생성·검토 입력에 예시 문항 2건(서술형 1·논술형 1)을 통째로 싣는다: 채점 기준표 전 요소 × 전 척도(0점부터)·총체적·유의점·예시답안 전부·출처, 건당 ≤3,500자(자료 글부터 줄임), + 짧은 카드 2장(`lib/reference/exemplars.ts` `exemplarFull`·`exemplarsBlockFull`; 은행의 새 하위 폴더(경기논술형 등)도 자동 후보). 과제에 형식 틀(성취기준 → 평가 요소 → 문항(전제문·<자료 n>·발문·[N점]) → 조건 → 채점 기준표(요소×척도, 총체적) → 예시답안 → 채점 시 유의점)과 "형식과 밀도를 따르되 문장·수치는 옮기지 않는다", 검토에 밀도 비교(척도 서술이 예시보다 얇으면 other). 5단계 생성 입력 수학 36.2k→39.1k·과학 37.2k→39.1k자(45k 미만 가드 `tests/prompts.test.ts`).
...
5. 5일차 검토(순서서 E절, 과목×12항목 체크리스트를 `docs/review/2026-09-29-owner-review.md`에 기록).
   ...
   - 2026-09-26: 제작소 3·4·6·7단계 탭은 요약 대신 원장·학생이 보는 완성본 그대로다(`components/studio/parts/` 조각을 원장 패키지 화면과 함께 씀) — 3단계 차시 카드 전부 + 차시마다 "교사용 지침" 칸(발문 대본·유의점·6단계 지침서의 그 차시 메모), 4단계 자료 전부(표 모든 행·그래프·이미지, 가리키는 대주제 공유 자료는 '공유' 표시), 6단계 지침서 전체, 7단계 안내장 틀 전체. 원장 패키지 화면의 차시 카드에도 같은 교사용 지침 칸이 붙는다(학생 화면에는 없음).
   - 2026-09-26: 관리자·원장이 읽는 화면(차시 카드·교사용 지침 칸·평가 계획·지침서·안내장 틀·문항 카드·제작소 3~7단계 탭, 모든 과목)은 내용을 옆으로 잇지 않는다 — 발문(굵게) → 아랫줄 예상 답 → 아랫줄 막힐 때(들여쓰고 옅게), 퀴즈 문제 → 정답 → 해설, 활동지는 층(기본/표준/도전)별 목록, 조건·분량·형식·유의점·A~E·준비물·오답 코멘트도 한 줄에 하나씩. 공용 틀은 `components/studio/parts/common.tsx`(SectionTitle·SubLabel·Lines·LabeledLines·KV·Aside). 학생 화면·문제지 인쇄 표식은 그대로.
