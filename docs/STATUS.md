...
- 2026-09-26 대표님: **퀴즈·차시 난이도를 성취수준 틀에 묶는다**("차시 설계나 퀴즈가 너무 쉬운 수준이 아닌지") — 교수 차시 퀴즈 3문항은 모두 단답형이되 D~E 회상(용어·사실) · C 이해·적용(도달점 수준의 적용·계산·설명 핵심어) · B 관계·추론(두 개념의 관계·까닭·새 사례) 하나씩(`QuizItem.level_ref`, 규칙 L-10 개정 — "워밍업 어투" 삭제), 자료·대본 문장을 옮겨 적으면 답이 되는 퀴즈 금지; 전개는 C(도달점) 목표 + A~B 학생용 확장 활동 1개(발문 또는 활동지 도전 과제, L-08). 새 세트는 zod(`LessonDesign`)가 교수 차시마다 수준 분포를 요구하고(`level_ref`는 선택 필드라 옛 저장본·게시 판은 그대로 읽힌다), [TS] 참고 메모 둘("N차시 퀴즈 3문항은 수준 D~E·C·B를 하나씩(level_ref)(지금 …)", "N차시 퀴즈 k: 정답이 본문에 그대로 있음(발문 대본·수업 흐름·활동지·자료 X)")은 막지 않는다. 수준은 교사용 — 원장·관리자 차시 카드의 교사용 지침 칸 "퀴즈 수준" 줄에만 보이고 학생 화면에는 없다. 시연 fixture: v1에서 올린 1~4차시 퀴즈가 발문 대본·활동지와 같은 문항(compat가 v1 퀴즈로 발문·활동지를 만든다)이라 퀴즈를 새 사례·수치로 다시 써(27문항, 발문·활동지는 수업용으로 그대로) 30문항 모두 수준을 붙였다(`scripts/upgrade-fixtures-v2.ts` `setLeveledQuizzes`). 주의: 이 변경 전에 3단계를 생성해 저장한 초안은 `level_ref`가 없어 새 세트 zod를 통과하지 못한다(차시 이미지 첨부·3단계 문장 고치기 저장이 "퀴즈 3문항은 수준 …"으로 거절됨) — 3단계를 다시 생성하면 된다. 게시·배정된 판은 영향 없다.
