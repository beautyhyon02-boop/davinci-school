...
export function buildNoticePrompt({ snapshot, lessonNo, skeleton, evidence = [] }: { snapshot: Snapshot; lessonNo: number; skeleton: NoticeT; evidence?: NoticeEvidence[] }) {
  ...
  const user = [
    ...
    `과제: 아래 안내장 뼈대의 빈 칸만 채워라 — 틀린 퀴즈의 note(40자, 부분 긍정+역접+완곡), 요소별 good_point(60자, 활동명+구체 행위+정도부사)와 improve_point(60자, 부분 긍정+역접+혼자 할 수 있는 다음 행동; 만점 요소는 null), 재도전이 있으면 improvement_comment(70자, 이전 점수를 출발점으로). 학부모와 학생이 함께 읽는다. 요소명은 채점표 그대로. 점수·정오·이름은 바꾸지 않는다. 다른 학생·등수·비교는 쓰지 않는다.`,
    ...
    evidence.length ? `확정 채점의 요소별 근거(원장 확정본):\n${evidence.map((e) => `- ${e.attempt}회차 ${e.criterion_name} ${e.points}/${e.max}: ${e.evidence}`).join('\n')}` : '',
