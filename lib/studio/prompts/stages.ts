...
const TASKS: Record<Stage, string> = {
  ...
  2: '재구조화 표(standards: 성취기준마다 통합/재조정/유지, original_text는 원문 그대로, reconstructed_text는 "학생은 [자료]를 가지고 [수행]을 해서 [결과물]을 할 수 있다", reason 태그, merged_with(재구조화 유형이 통합이면 함께 묶은 다른 성취기준 코드 배열, 그 외는 빈 배열), learning_elements; reconstructed_text는 original_text와 merged_with 성취기준 원문에 있는 어휘만 쓴다)와 세트 통합 문장 1개(reconstruction, 같은 틀로 세트 성취기준 원문에 있는 어휘만 쓴다), 학습 목표 3~5개(각각 axis=지식·이해/과정·기능/가치·태도, 세 축 모두 1개 이상), 세트 핵심질문 후보 2~3개를 만들어라. 재구성 문장 두 가지(성취기준마다의 reconstructed_text와 통합 문장 reconstruction)는 모두 성취기준 원문의 낱말과 문장 틀 낱말(학생은·가지고·해서·할 수 있다·것·위해·통해 같은 기능어)만 쓴다 — 위 대주제의 상황(학교 축제·일회용품처럼 대주제 제목에 나오는 낱말, 안내문·도표의 구체적인 주제)은 재구성 문장에 쓰지 않고, 학습 목표(learning_goals)·세트 핵심질문 후보와 3단계 차시에서만 쓴다. 위에 준 C 문장(도달점)보다 좁아지거나 다른 활동을 가리키지 않게 한다. level_anchor는 빈 배열로 둔다(서버가 채운다).',
