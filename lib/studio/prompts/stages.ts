...
const TASKS: Record<Stage, string> = {
  ...
  4: '차시와 문항에 필요한 가상 자료를 만들어라. 표는 열·행으로, 설명글은 본문으로. 수치는 합계와 비율이 맞아야 한다. 공유 자료가 주어지면 그 수치를 그대로 쓴다. 자료는 원자료만 담는다 — 학생이 문항에서 만들어야 할 결과(정리된 도수분포표·계산된 상대도수·평균 같은 값)를 body나 표에 미리 적지 않는다.',
  5: '서술형 2개(각 3점)와 논술형 1개(16점)를 만들어라. 문두는 조건+평가요소+기능+배점. 학생은 노트북 화면의 글 입력 칸에 키보드로 답을 쓴다 — 그래프를 그리게 하지 말고 해석만 묻고, 표를 요구할 때는 "각 줄에 계급: 도수처럼 쓰고 마지막 줄에 합계"처럼 글로 쓸 수 있는 형식을 conditions에 적는다. 학생은 문두와 conditions만 보고 답안을 쓰므로 conditions는 구체적인 작성 지침이어야 한다: length는 글자 수·문장 수·표 행 수처럼 셀 수 있는 분량("제한 없음" 금지), required는 3~5개로 어느 자료의 어떤 수치·용어를 몇 개 넣을지와 답안의 순서(① 무엇을 먼저 ② 그다음)까지 적고, format은 표/문장/문단·종결어미("~다")·단위 표기를 명시한다. 각 문항의 채점표, 등급 경계표(총 22점, 7등급), 논술형 예시답안 상/중/하(요소별 점수·총점·등급 포함), 피드백 문구 틀을 만들어라.',
...
const REVIEW_FOCUS: Record<Stage, string> = {
  ...
  5: '채점표로 예시답안 상/중/하를 실제로 채점했을 때 적힌 점수·등급이 나오는지(rubric). 배점 합계 22, 등급표 일치. 문두가 조건+평가요소+기능+배점인지. conditions가 학생 혼자 답안을 쓸 수 있을 만큼 구체적인지(length가 셀 수 있는 분량인지, required 3~5개에 자료·수치·순서가 있는지, "제한 없음" 같은 빈 조건이 없는지 — 있으면 kind other). 성취기준 이탈 여부.',
