...
const REVIEW_FOCUS: Record<Stage, string> = {
  ...
  3: `모든 성취기준이 어느 차시엔가 배정됐는지(coverage). 교수 차시마다 퀴즈 3문항(마지막 교수 차시 포함)이 차시 핵심질문을 점검하고 정답이 맞으며, 정답이 낱말·수치·짧은 구라 표기가 조금 달라도 맞힐 수 있는지(quiz). 퀴즈가 모두 단답형(type "short", choices null)인지 — 선택지가 있거나 "다음 중"처럼 보기를 고르는 객관식 문항이 있으면 other(서논술 과정이라 객관식은 없다). 교수 차시 퀴즈 3문항의 level_ref가 D~E(회상)·C(이해·적용)·B(관계·추론) 하나씩이고 문항이 실제로 그 수준인지 — 세 문항이 모두 용어·사실 회상이거나 B라고 붙인 문항이 낱말 하나 떠올리기면 level. 정답이 자료 본문·발문 대본·전개 문장에 그대로 적혀 있어 옮겨 적으면 되는 문항, 발문·활동지 문항을 그대로 되풀이한 퀴즈는 quiz. 전개 활동이 C(도달점)를 목표로 하고 A~B 학생용 확장 활동(발문 또는 활동지 도전 과제)을 1개 이상 두었는지 — 없으면 level. ${SET_KINDS}이 마지막 교수 차시 뒤 단원 평가 차시(kind "assessment", ${SESSION_STEPS})에 함께 있고 교수 차시에는 서·논술형이 없는지(coverage). 발문이 원장이 읽고 그대로 진행할 만큼 구체적이고 if_stuck이 정답을 그대로 말하지 않는지. 교수 차시 활동지 기본·표준·도전이 실제로 난이도 차이가 나는지(level). caution_notes에 오개념이 있는지. 시간 배분이 활동량과 맞는지. 교수 차시의 수업·퀴즈가 단원 평가 문항의 답을 미리 말하지 않는지(C-03). 차시 materials_used가 이 과목 수업과 무관한 대주제 공유 자료를 가리키는지(other) — 이 과목 활동지·발문·퀴즈 어디에도 쓰이지 않는 공유 자료를 단지 대주제가 공유한다는 이유로 적어 넣었으면 짚는다.`,
