...
/** 5단계 참고 카드: 평가원·교육청 공개 예시 문항 수준을 맞추려고 종류별로 뽑는다(서술형 2 + 논술형 3). */
...
const REVIEW_FOCUS: Record<Stage, string> = {
  ...
  3: '모든 성취기준이 어느 차시엔가 배정됐는지(coverage). 교수 차시마다 퀴즈 3문항(마지막 교수 차시 포함)이 차시 핵심질문을 점검하고 정답이 맞는지(quiz). 서술형 1 + 논술형 1이 마지막 교수 차시 뒤 단원 평가 차시(kind "assessment", 서술형 작성 15분 + 논술형 작성 35분)에 함께 있고 교수 차시에는 서·논술형이 없는지(coverage). 발문이 원장이 읽고 그대로 진행할 만큼 구체적이고 if_stuck이 정답을 그대로 말하지 않는지. 교수 차시 활동지 기본·표준·도전이 실제로 난이도 차이가 나는지(level). caution_notes에 오개념이 있는지. 시간 배분이 활동량과 맞는지. 교수 차시의 수업·퀴즈가 단원 평가 문항의 답을 미리 말하지 않는지(C-03).',
  ...
  5: '문항마다 예시답안을 채점표로 실제로 채점해 적힌 요소별 점수·총점이 나오는지, 논술형 상/중/하 예시답안 총점에 assumed_short_points(그 예시가 전제한 서술형 문항 점수 0~6)를 더한 세트 총점이 등급표에서 각 밴드에 떨어지는지(rubric; 불일치면 어느 요소가 몇 점 차이인지 detail에 적는다). 서술형 예시답안의 assumed_short_points가 null인지. 문항 lesson_no가 3단계 unit_plan.assessment_plan.summative_placement의 차시(단원 평가 차시)와 같은지(coverage). 문항 2개 = 서술형 6 + 논술형 16 = 22, 등급표 일치. 두 문항 모두 총체적 상/중/하가 있고 요소별 점수와 어긋나지 않는지(rubric, C-15). 서술형 예시답안이 1~6점 단계마다 있는지. 두 문항이 평가원·교육청 공개 예시 문항 수준·형식인지(발문·자료·채점표의 밀도). 문두가 전제문+발문+[배점]이고 사고 순서 = 조건 순서인지. 서술형에 조건(items)이 있으면 other, 논술형 조건이 2~4개가 아니면 other. 조건이 풀이 과정·공식·수치·순서를 담고 있으면 other(조건은 지침만). 지침은 입장·근거 개수·인용할 자료·형식·분량·초과 응답 규칙이다(C-32). length가 셀 수 있는 분량이고 format이 표/문장/문단·종결어미를 밝히는지("제한 없음" 같은 빈 분량이 있으면 other). 척도 descriptor가 관찰 가능한 표현인지, 인접 단계가 부사만 다른지(level). 논술형에 가치·태도 축 요소가 있으면 정당화 가능성 기준인지. 예시 은행 문장을 그대로 베끼지 않았는지(source). 성취기준 이탈 여부. 8문항 자가 점검(성취기준 부합·3범주 반영·상황맥락·고차 사고·채점기준 부합·변별·명료성·채점자 불변성).',
