...
const REVIEW_FOCUS: Record<Stage, string> = {
  ...
  4: '수치 자료의 합계·비율이 맞는지. 자료가 답을 대신하지 않는지(문항이 요구할 정리·계산 결과나 결론 문장이 있으면 other). 찬반·비교 자료의 균형. 학교급(학년군) 어휘 수준. 표가 한 화면(25행·6열)인지. source가 자작인지. 세트 자료가 문항·차시가 실제로 쓰는 2~4개뿐인지 — 3단계 차시의 materials_used가 가리키지 않는 자료를 늘어놓았거나 공유 자료와 합쳐 5개를 넘으면 other(세트 자료는 문항·차시가 실제로 쓰는 2~4개만 만든다; 공유 자료는 이 과목 문항이 인용할 것만 materials_used에 넣고 나머지는 쓰지 않는다). 자료 안의 지시·사실이 서로 모순되거나 지시 대상이 모호한 문장(예: 가져온 컵을 반납하라)이 있으면 other.',
