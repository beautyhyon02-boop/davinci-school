...
const TASKS: Record<Stage, string> = {
  ...
  5: '서술형 2개(각 3점)와 논술형 1개(16점)를 만들어라. 문두는 조건+평가요소+기능+배점. 학생은 문두와 conditions만 보고 답안을 쓰므로 conditions는 구체적인 작성 지침이어야 한다: length는 글자 수·문장 수·표 행 수처럼 셀 수 있는 분량("제한 없음" 금지), required는 3~5개로 어느 자료의 어떤 수치·용어를 몇 개 넣을지와 답안의 순서(① 무엇을 먼저 ② 그다음)까지 적고, format은 표/문장/문단·종결어미("~다")·단위 표기를 명시한다. 각 문항의 채점표, 등급 경계표(총 22점, 7등급), 논술형 예시답안 상/중/하(요소별 점수·총점·등급 포함), 피드백 문구 틀을 만들어라.',
