...
가짜 응답(mock) 수학 세트는 **2차시 = 서술형1, 4차시 = 서술형2, 5차시 = 논술형**이고 1·3차시는 퀴즈만 있습니다.

1. **원장**으로 로그인
2. `/teacher/students` → 학생 2명 추가 (초기 비밀번호 저장)
3. `/teacher/items` → 수학 세트 열기 → [배정하기] → 학생 선택 → "처음 열 차시 수" 2 → "재도전 허용" 체크 → 배정
4. **학생** 계정으로 로그인(첫 번째 학생)
5. `/student` → 배정된 세트 열기 → 1차시 퀴즈 제출 → 즉시 결과 확인
6. 2차시 → 퀴즈 제출 → 서술형1 답안 60자 이상 제출 → "확인 중" 상태 확인
7. **원장** 계정으로 로그인
8. `/teacher/assignments` → 세트 [열기] (`/teacher/assignments/[setId]`) → 서술형1 검수 대기 카드 → 점수 확인·수정 → 확정
9. 같은 화면 "차시 열기"에서 "5차시까지 열기" 선택 → [반 전체에 적용]
10. **학생** 계정으로 로그인(첫 번째 학생) → 4차시 서술형2, 5차시 논술형 답안 제출
11. **원장** 계정으로 로그인 → 두 카드 확정 (세 문항이 모두 확정되면 종합 점수·등급이 나옵니다)
