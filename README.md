...
3. `/teacher/items` → 수학 세트 선택 → [배정하기] → 학생 선택 → "3차시까지 열기" → 배정
4. **학생** 계정으로 로그인(첫 번째 학생)
5. `/student` → 배정된 세트 열기 → 1차시 퀴즈 제출 → 즉시 결과 확인
6. 3차시 → 서술형·논술형 각 60자 이상 답안 제출 → "확인 중" 상태 확인
7. **원장** 계정으로 로그인
8. `/teacher/assignments` → 검수 대기 카드 클릭 → 점수 확인·수정 → 확정
9. **학생** 계정으로 로그인(첫 번째 학생)
10. `/student` → 세트 열기 → 결과 확인 → [다시 써 보기] → 2회차 제출
11. **원장** 계정으로 로그인 → 2회차 확정
12. **학생** 계정으로 로그인(첫 번째 학생) → 1회차·2회차 비교 확인
